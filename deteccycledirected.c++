...
public:
    ...
    bool dfs(int i, int **adj, int *vis, int *mstate, int V)
    {
        vis[i] = 1;
        mstate[i] = 1;
        ...
        for (int j = 0; j < V; ++j)
        {
            ...
            { // Check if there's an edge from i to j
                ...
                {
                    // Cycle detected since mstate[j] is still 1
                    return true;
                }
            }
        }

        mstate[i] = 0; // Backtrack: remove node from current path
        return false;
    }
